...
const createTables = () => {
  ...
  // Petty Cash Requisitions table
  db.prepare(`
    CREATE TABLE IF NOT EXISTS petty_cash_requisitions (
      id TEXT PRIMARY KEY,
      amount REAL NOT NULL,
      amount_in_words TEXT NOT NULL,
  ...
  // Form Approvals table (for expense claims, EFT forms, and petty cash)
  db.prepare(`
...
const getPettyCashRequisitions = () => db.prepare('SELECT * FROM petty_cash_requisitions ORDER BY created_at DESC').all();
const getPettyCashRequisitionById = (id) => db.prepare('SELECT * FROM petty_cash_requisitions WHERE id = ?').get(id);
const createPettyCashRequisition = (requisition) => {
  return db.prepare(`
    INSERT INTO petty_cash_requisitions (id, amount, amount_in_words, payee_name, purpose,
                                         description, department, initiator_id, initiator_name, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(requisition.id, requisition.amount, requisition.amount_in_words, requisition.payee_name,
         requisition.purpose, requisition.description || null, requisition.department,
         requisition.initiator_id, requisition.initiator_name, requisition.status);
};
const updatePettyCashRequisitionStatus = (id, status) => {
  return db.prepare('UPDATE petty_cash_requisitions SET status = ? WHERE id = ?').run(status, id);
};

// Petty Cash Line Items
const getPettyCashItems = (requisitionId) => {
  return db.prepare('SELECT * FROM petty_cash_items WHERE requisition_id = ? ORDER BY item_no ASC')
    .all(requisitionId);
};
const createPettyCashItem = (item) => {
  return db.prepare(`
    INSERT INTO petty_cash_items (requisition_id, item_no, description, amount)
    VALUES (?, ?, ?, ?)
  `).run(item.requisition_id, item.item_no, item.description, item.amount);
};
const updatePettyCashItem = (id, item) => {
  return db.prepare(`
    UPDATE petty_cash_items
    SET description = ?, amount = ?
    WHERE id = ?
  `).run(item.description, item.amount, id);
};
const deletePettyCashItem = (id) => {
  return db.prepare('DELETE FROM petty_cash_items WHERE id = ?').run(id);
};
